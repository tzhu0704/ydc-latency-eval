...
For concurrent endpoint testing, use `run_concurrent_matrix.py`. It runs query/endpoint cells concurrently, records raw JSONL samples, and generates endpoint-level JSON and TSV summaries:

```bash
export YDC_API_KEY='your-api-key'
python3 run_concurrent_matrix.py \
  --queries query_sets/sea-1.json query_sets/sea-2.json query_sets/sea-3.json \
  --rounds 10 --concurrency 4 --report-prefix reports/use1-vs-ydc-search
```

With no `--endpoint` arguments, the runner compares the requested four-cell matrix: `use1.ydc-index.io/search`, `ydc-index.io/search`, `use1.ydc-index.io/v1/search`, and `ydc-index.io/v1/search`. To use different credentials or add targets, repeat `--endpoint HOST/PATH@API_KEY_ENV`. Use the same API key for all four cells when the goal is a fair endpoint/path latency comparison.

Each request disables connection reuse and sends `Cache-Control: no-cache, no-store`, `Pragma: no-cache`, and a unique client request ID. These reduce client/intermediary cache effects; they cannot guarantee that a server-side application cache ignores the request.

Before performance measurement, the runner performs a functional smoke check for every endpoint/path and stops if any check fails. It writes `<prefix>.functional.json`; use `--skip-functional-check` only when intentionally testing an endpoint that does not return the standard Search response shape.

Generate a self-contained English HTML report after a run:

```bash
python3 generate_html_report.py \
  --report-prefix reports/use1-vs-ydc-100 \
  --output reports/use1-vs-ydc-100.html
```

`--queries` accepts multiple JSON query-set files. Round 1 uses the first set, round 2 the second, and so on; if there are fewer sets than rounds, the sets are reused in rotation. For each round, all configured endpoints receive the same question population, allowing endpoint latency to be compared under equivalent queries. The query-set files are not generated by the runner; prepare them beforehand as arrays of `{\"id\": \"...\", \"query\": \"...\"}` objects.
